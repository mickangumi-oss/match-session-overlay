...
FRIENDS／FOLLOWINGは管理画面を表示している間、設定した戦績取得間隔と同じ周期で自動更新します。フレンドのオンライン通知がOFFの場合、管理画面を閉じて通知領域へ格納している間は更新を停止し、再表示時にすぐ更新します。通知がONの場合は、管理画面を閉じていてもFRIENDSのみを同じ周期で確認します。
