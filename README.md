...
通知タイミングを「ゲーム起動中のみ」にする場合は、アプリ設定で対象ゲームの実行ファイル（.exe）を登録してください。未登録の場合はゲームの起動状態を判定できないため、ゲームを起動していても通知されません。「ゲーム起動検知」をONにする必要はありません。未登録中に発生したオンラインへの変化を、あとから遡って通知することもありません。
