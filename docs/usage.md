...
![直近5件を表示した管理画面の例（仮データ）](images/management-history-example.jpg)
...
「コンピューターの起動時にアプリを実行する」がONの場合、Windowsサインイン時は管理画面を表示せず、通知領域（システムトレイ）に常駐します。トレイアイコンをダブルクリックするか、右クリックメニューの「管理画面を表示」で画面を開けます。管理画面を最小化または閉じると、戦績ウィンドウとOBS用オーバーレイも非表示になります。トレイメニューの「終了」を選ぶとアプリとオーバーレイを終了します。
